...
export interface PlayerEliminationBannerEvent {
  playerId: string;
  playerName: string;
  texture: string;
  frame: string;
...
export interface EliminationBannerOptions {
  depth?: number;
  height?: number;
  margin?: number;
  durationMs?: number;
...
export class EliminationBanner {
  ...
  constructor(scene: Phaser.Scene, options: EliminationBannerOptions = {}) {
    this.scene = scene;
  ...
  show(payload: PlayerEliminationBannerEvent): void {
    this.portrait.setTexture(payload.texture, payload.frame);
    this.label.setText(`Player ${payload.playerName} eliminated`);
  ...
  layout(width: number): void {
    ...
    const textX = imageX + portraitSize / 2 + this.margin;
